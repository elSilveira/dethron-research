"""Frozen V1 custody run: proof of entry from relays, proof of exit from the recipient."""
import hashlib
...
def main():
    ...
    root = base/'results'/f'gateway-v1-{time.time_ns()}'
    root.mkdir(parents=True)
    print(f'Artifacts: {root}', flush=True)
    files = list(base.glob('v1_*.py'))+list((base/'dethron_gateway').glob('*.py'))
    files += [base/name for name in ('run_v1_probe.py', 'g3_process.py', 'g2_receiver.py',
    ...
    try:
        ...
        for scenario in PROFILE['scenarios']:
            ...
            if case['audit']['completed'] != EXPECTED[scenario]:
